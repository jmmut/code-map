...
async fn main() -> Result<(), AnyError> {
    ...
    loop {
        ...
        let mut available = Rect::new((width * 0.05).round(), (height * 0.05).round(), (width * 0.9).round(), (height * 0.75).round());

        draw_rectangle_lines((width * 0.05).round(), (height * 0.05).round(), available.w, available.h, 1.0, BLACK);


        // if is_mouse_button_pressed(MouseButton::Left) {
        let mouse_position = Vec2::from(mouse_position());
        let virtual_position = Vec2::new((mouse_position.x - available.x) / available.w, (mouse_position.y - available.y) / available.h);
        if Rect::new(0.0, 0.0, 1.0, 1.0).contains(virtual_position) {
            ...
            draw_text(&text, available.x + 1.5 * font_size, available.y + available.h + 5.0 * font_size, font_size, BLACK);
        }
        // }
